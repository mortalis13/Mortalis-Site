...
jQuery(function($) {
  ...
  $(window).scroll(function() {
    ...
    if (intersect) {
      // if there is apossibility of intersection (overlapping)
    ...
    if (restore && bottomChanged) {
      if (intersect) {
        // no animation if button restores its 'bottom' after scrolling up and if no overlapping
        toTop.css('bottom', origBottom)
        bottomChanged = false
      }
      else {
        // animates when the button goes down if the window is resized and scrolled
